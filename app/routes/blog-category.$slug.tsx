...
export let loader: LoaderFunction = async ({ request, params }) => {
    try {
        ...
        const bcdetailCacheKey = `blogcategory-${params.slug}`;
        const cachedBCDetail = cache[bcdetailCacheKey];
        let blogs;
        if (!cachedBCDetail) {
            const bc_detail = await fetch(config.apiBaseURL + 'blogcategory/' + params.slug);
            if (!bc_detail.ok) { throw bc_detail; }
            blogs = await bc_detail.json();
            cache[bcdetailCacheKey] = blogs;
            setTimeout(() => {
                delete cache[bcdetailCacheKey];
            }, CACHE_EXPIRATION_TIME);
        } else {
            blogs = cachedBCDetail;
        }
            ...
            <div className="container mx-auto">
                <div className="py-3">
                    <div className="flex items-center py-2 text-sm font-normal">
                        <Link title="Home" to="/" className="text-sm font-normal text-[#131B23]">Home</Link> &nbsp;<i className="fa fa-chevron-right text-[10px]"></i><i className="fa fa-chevron-right text-[10px]"></i>&nbsp; <Link to="/blogs" className="text-sm font-normal text-[#131B23]">Blogs</Link> &nbsp;<i className="fa fa-chevron-right text-[10px]"></i><i className="fa fa-chevron-right text-[10px]"></i>&nbsp;  <div className="text-sm font-normal text-theme underline">{blogs.data.title}</div>
                    </div>
                    <div className="py-3">
                                        ...
                                        <div>
                                            {blogcategories.data.data.map((blogcategory: any, index: any) => (
                                                <div className="text-lg" key={index}>
                                                    <Link title={blogcategory.title} to={'/blog-category/' + blogcategory.slug} className={`text-lg leading-10 text-normal py-1 ${blogcategory.slug == blogs.data.slug ? 'text-theme ' : 'text-[#131B23] '}`}>{blogcategory.title} ({blogcategory.blogCount})</Link>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
